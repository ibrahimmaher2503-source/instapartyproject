...
final class RejectWithdrawalTransition extends Transition
{
    public function __construct(
        private readonly Withdrawal $model,
        private readonly int $actorId,
        private readonly ?array $rejectedReason = null,
    ) {}
    ...
    public function handle(): Withdrawal
    {
        abort_unless(
            ...
            403,
            'Insufficient permissions to reject withdrawal'
        );
        ...
        if ($this->rejectedReason !== null) {
            $this->model->rejected_reason = $this->rejectedReason;
        }
        $this->model->save();

        $this->model->status->transitionTo(RejectedState::class);
