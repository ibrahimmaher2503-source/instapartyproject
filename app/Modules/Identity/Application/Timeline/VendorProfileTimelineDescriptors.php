...
namespace App\Modules\Identity\Application\Timeline;
...
final class VendorProfileTimelineDescriptors
{
    ...
    public static function all(): array
    {
        return [
            ...
            self::vendorDocumentsDescriptor(),
    ...
    private static function stateTransitionsDescriptor(): SourceTableDescriptor
    {
        return new SourceTableDescriptor(
            ...
            canonicalRank: 10,
            queryBuilder: function (VendorProfile $subject, TimelineSubjectScope $scope, TimelineFilters $filters) {
                $query = StateTransition::query()
                    ->where('transitionable_type', VendorProfile::class)
                ...
                if ($filters->eventKind !== null && $filters->eventKind !== TimelineEventKind::StateChange) {
                    // This descriptor only emits StateChange events — return empty if filtering by another kind
                    $query->whereRaw('0 = 1');
                }
            ...
            rowMapper: function (StateTransition $row, TimelineAudience $audience): TimelineEntryDTO {
                return new TimelineEntryDTO(
                    ...
                    actionKey: 'vendor.state_changed',
                    ...
                    isAdminOnly: false,
                    extra: [
                        '_state_class' => VendorApprovalState::class,
                        '_id' => $row->id,
                    ],
                );
            },
    ...
    private static function vendorDocumentsDescriptor(): SourceTableDescriptor
    {
        return new SourceTableDescriptor(
            sourceTable: 'vendor_documents',
            defaultEventKind: TimelineEventKind::Document,
            canonicalRank: 20,
            queryBuilder: function (VendorProfile $subject, TimelineSubjectScope $scope, TimelineFilters $filters) {
                $query = DB::table('vendor_documents')
                    ->where('vendor_profile_id', $subject->getKey())
                    ->select([
                        'id',
                        'public_id',
                        'doc_type',
                ...
                if ($filters->eventKind !== null && $filters->eventKind !== TimelineEventKind::Document) {
                    $query->whereRaw('0 = 1');
                }

                // Documents have no actor_role on the row itself; skip actorRole filter
                return $query;
            },
            rowMapper: function (object $row, TimelineAudience $audience): TimelineEntryDTO {
                $actionKey = match ($row->status) {
                    'approved' => 'vendor.document_approved',
                    'rejected' => 'vendor.document_rejected',
                    default => 'vendor.document_uploaded',
                };

                $rejectionNote = null;
                if ($row->status === 'rejected' && ! empty($row->review_notes)) {
                    $decoded = is_string($row->review_notes)
                        ? json_decode($row->review_notes, true)
                        : (array) $row->review_notes;
                    $rejectionNote = $decoded[app()->getLocale()] ?? $decoded['en'] ?? null;
                }
                ...
                return new TimelineEntryDTO(
                    ...
                    toState: $row->status,
                    note: $rejectionNote,
                    eventKind: TimelineEventKind::Document,
                    isAdminOnly: false,
                    extra: [
            ...
            visibilityRules: [
                ...
                'vendor' => [
                    'columns' => ['doc_type', 'status', 'review_notes'],
                    'jsonKeys' => [],
                    'stripDeep' => [],
                    'adminOnly' => false,
                ],
    ...
    private static function auditLogsDescriptor(): SourceTableDescriptor
    {
        return new SourceTableDescriptor(
            ...
            queryBuilder: function (VendorProfile $subject, TimelineSubjectScope $scope, TimelineFilters $filters) {
                ...
                if ($filters->actorRole !== null) {
                    // audit_logs has no trigger_kind; only System is emitted — skip if filtering by non-system
        ...
        return new SourceTableDescriptor(
            ...
            queryBuilder: function (VendorProfile $subject, TimelineSubjectScope $scope, TimelineFilters $filters) {
                return DB::table('activity_log')
                    ->where('subject_type', VendorProfile::class)
                    ->where('subject_id', $subject->getKey());
            },
            rowMapper: function (object $row, TimelineAudience $audience): TimelineEntryDTO {
                $causerName = null;
                if (! empty($row->causer_id) && ! empty($row->causer_type)) {
                    // Resolve lazily — if the name is stored on the log row use it directly
                ...
                return new TimelineEntryDTO(
                    ...
                    actionKey: 'vendor.profile_updated',
                    fromState: null,
                    toState: null,
