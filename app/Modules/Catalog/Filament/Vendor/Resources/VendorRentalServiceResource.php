...
use App\Modules\Catalog\Filament\Vendor\Resources\VendorRentalServiceResource\Pages\CreateVendorRentalService;
use App\Modules\Catalog\Filament\Vendor\Resources\VendorRentalServiceResource\Pages\EditVendorRentalService;
use App\Modules\Catalog\Filament\Vendor\Resources\VendorRentalServiceResource\Pages\ListVendorRentalServices;
...
class VendorRentalServiceResource extends Resource
{
    ...
    {
        return __('vendor-portal.services.rental_services');
    }
    ...
    public static function form(Form $schema): Form
    {
        return $schema->components([
            ...
            Section::make(__('catalog.rental_details'))
                ->schema([
                    Grid::make(2)->schema([
                        Toggle::make('rentalDetail.requires_electricity')
                            ->label(__('catalog.requires_electricity'))
                            ->default(false),
                        Toggle::make('rentalDetail.requires_outdoor_space')
                            ->label(__('catalog.requires_outdoor_space'))
                            ->default(false),
                    ]),
                    Grid::make(2)->schema([
                        TextInput::make('rentalDetail.default_rental_duration_hours')
                            ->label(__('catalog.default_rental_duration_hours'))
                            ->numeric()
                            ->required()
                            ->minValue(1),
                        TextInput::make('rentalDetail.setup_time_minutes')
                            ->label(__('catalog.setup_time_minutes'))
                            ->numeric()
                            ->minValue(0),
                    ]),
                    Grid::make(2)->schema([
                        TextInput::make('rentalDetail.teardown_time_minutes')
                            ->label(__('catalog.teardown_time_minutes'))
                            ->numeric()
                            ->minValue(0),
                        TextInput::make('rentalDetail.security_deposit_minor')
                            ->label(__('catalog.security_deposit'))
                            ->numeric()
                            ->minValue(0)
                            ->suffix(__('catalog.piastres_suffix')),
                    ]),
                    TextInput::make('rentalDetail.minimum_space_sqm')
        ...
        return $table
            ...
            ->actions([
                ...
                Action::make('manageAvailability')
                    ...
                    ->color('info')
                    ->url(fn (Service $record): string => VendorServiceAvailabilityPage::getUrl(
                        ['service' => $record->public_id],
                        panel: 'vendor',
                    )),
    ...
    {
        return [
            'index' => ListVendorRentalServices::route('/'),
            'create' => CreateVendorRentalService::route('/create'),
            'edit' => EditVendorRentalService::route('/{record}/edit'),
        ];
    }
