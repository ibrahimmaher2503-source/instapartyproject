...
Read the relevant section before changing application code. Root AGENTS.md governs instruction precedence, missing-document handling, and current checkout facts. These retained constraints describe intended policy, not proof that the current code complies. Referenced specs and architecture tests are currently missing; see checkout-gaps.md. Commands require installed development tools.
...
7. **Run `php vendor/bin/pint` and `./vendor/bin/phpstan analyse` before every commit.** All green required.
